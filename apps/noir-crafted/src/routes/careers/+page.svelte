<!--
  Jxion Stack — Careers Page
  Phase Reference: Phase 3 — Template Composition & Page Assembly
  Description: Careers page using Jxion components with dynamic content
-->
...
<script lang="ts">
  ...
  $: sectionProps = contentData?.careers ? {
    title: contentData.careers.title || 'Kariyer',
    subtitle: contentData.careers.subtitle || '',
    description: contentData.careers.description || '',
    content: contentData.careers.content || '',
    footerContent: contentData.careers.footerContent || '',
    variant: 'default',
    size: 'large',
  } : null;
...
<svelte:head>
  <title>Kariyer - Noir Crafted</title>
</svelte:head>
