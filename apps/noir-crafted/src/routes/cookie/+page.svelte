...
<script lang="ts">
  ...
  $: sectionProps = contentData?.cookie ? {
    title: contentData.cookie.title || 'Çerez Politikası',
    subtitle: contentData.cookie.subtitle || '',
    description: contentData.cookie.description || '',
    content: contentData.cookie.content || '',
    footerContent: contentData.cookie.footerContent || '',
    variant: 'default',
    size: 'large',
  } : null;
