...
<script lang="ts">
  ...
  $: sectionProps = content?.privacy ? {
    title: content.privacy.title || 'Gizlilik Politikası',
    subtitle: content.privacy.subtitle || '',
    description: content.privacy.description || '',
    content: content.privacy.content || '',
    footerContent: content.privacy.footerContent || '',
    variant: 'default',
    size: 'large',
  } : null;
