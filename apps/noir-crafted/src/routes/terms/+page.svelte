<!--
  Jxion Stack — Terms Page
  Phase Reference: Phase 3 — Template Composition & Page Assembly
  Description: Terms page using Jxion components with dynamic content
-->
...
<script lang="ts">
  ...
  $: sectionProps = contentData?.terms ? {
    title: contentData.terms.title || 'Kullanım Koşulları',
    subtitle: contentData.terms.subtitle || '',
    description: contentData.terms.description || '',
    content: contentData.terms.content || '',
    footerContent: contentData.terms.footerContent || '',
    variant: 'default',
    size: 'large',
  } : null;
...
<svelte:head>
  <title>Kullanım Koşulları - Noir Crafted</title>
</svelte:head>
