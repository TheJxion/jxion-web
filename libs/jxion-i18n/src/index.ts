...
// Translation cache with stale-while-revalidate support
...
const translationCache: Map<string, CacheEntry> = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes
const STALE_TTL = 30 * 60 * 1000; // 30 minutes (stale but usable)

// Import backend client
import {
  fetchTranslationFromBackend,
  fetchTranslationsFromBackend,
  updateTranslationInBackend,
  updateTranslationsInBackend,
  clearCacheInBackend,
  configureBackend,
  type BackendConfig,
} from './backend-client';
...
async function loadTranslations(locale: Locale): Promise<Record<string, any>> {
  ...
  if (translationCache.has(cacheKey)) {
    ...
    if (age < STALE_TTL) {
      console.log(
        `[Jxion-i18n] Cache HIT (stale): ${locale} - revalidating in background`
      );
  ...
  if (backendData) {
    ...
    // Fallback to file-based - use getDictionary which loads from dictionary folder
    try {
      const { getDictionary } = await import('./get-dictionary');
      translations = (await getDictionary(locale)) as Record<string, any>;
      console.log(`[Jxion-i18n] Loaded from dictionary: ${locale}`);
    } catch (error) {
      console.error(
        `[Jxion-i18n] Failed to load translations for locale: ${locale}`,
        error
      );
 ...
 */
async function revalidateTranslations(locale: Locale): Promise<void> {
  try {
    // In production, fetch from backend
    // For now, just refresh from dictionary
    const { getDictionary } = await import('./get-dictionary');
    const translations = (await getDictionary(locale)) as Record<string, any>;
...
export const getTranslation = async (
  ...
  try {
    ...
    if (value === undefined) {
      console.warn(
        `[Jxion-i18n] Translation key not found: ${key} (locale: ${locale})`
      );
      return key;
    }
    ...
    console.error(
      `[Jxion-i18n] Error getting translation for key: ${key}`,
      error
    );
    return key;
  }
...
export const getTranslations = async (
  ...
  try {
    ...
    // Always load dictionary as base (for merging)
    const translations = await loadTranslations(locale);
    const result: Record<string, string> = {};

    // First, populate with dictionary values (base translations)
    for (const key of keys) {
      const value = getNestedValue(translations, key);
      result[key] = value !== undefined ? String(value) : key;
    }

    // Then, merge in backend translations (overrides dictionary for saved translations)
    if (backendData) {
      for (const [key, value] of Object.entries(backendData)) {
        // Only use backend value if it's not a placeholder (value !== key)
 ...
 */
export const useTranslation = (locale: Locale = 'tr-TR') => {
  return {
 ...
 */
export const clearTranslationCache = async (locale?: Locale): Promise<void> => {
  if (locale) {
 ...
 */
export const configureI18nBackend = (config: BackendConfig): void => {
...
export const updateTranslations = async (
  translations: Array<{ key: string; locale: Locale; value: string }>
): Promise<boolean> => {
